...
/**
 * Controller de la page modification d'une séance
 * @see ControllerSeance
 * @author Etienne Tillier, Romain Frezier
 */
public class ControllerModifySeance extends ControllerSeance{
    ...
    private void initialize() {
        ...
        try {
            ...
            prixSeance.setValue(seance.getPrix());
            montantSeance.setText(String.valueOf(seance.getPrix()));
            sportSeance.setValue(seance.getSport());
    ...
    /**
     * Méthode appelée lors du clic sur le bouton "Modifier". Modifie la séance
     */
    @FXML
    private void updateSeance() {
        try {
            if (verifChamps()){
                hideError(errorText);
                List<Triple<Exercice,Integer,Integer>> exercicesEnMoin = seance.getListeExercice().stream().filter(exercice -> !listeExerciceSeance.contains(exercice)).toList();
                for (Triple<Exercice,Integer,Integer> exercice : exercicesEnMoin) {
                    try {
                        facadeSeance.deleteExerciceFromSeance(exercice.getFirst(), seance.getId());
                    } catch (Exception e) {
                        e.printStackTrace();
                        displayError(errorText, e.getMessage());
                    }
                }
                List<Triple<Exercice,Integer,Integer>> exerciceEnPlus = listeExerciceSeance.stream().filter(exercice -> !seance.getListeExercice().contains(exercice)).toList();
                for (Triple<Exercice,Integer,Integer> exercice : exerciceEnPlus) {
                    try {
                        facadeSeance.addExerciceToSeance(exercice.getFirst(), exercice.getSecond(), exercice.getThird(), seance.getId());
                    } catch (Exception e) {
                        e.printStackTrace();
                        displayError(errorText, e.getMessage());
                    }
                }
                facadeSeance.updateSeance(seance.getId(),
                        nomSeance.getText(),
                        descriptionSeance.getText(),
                        (int) prixSeance.getValue(),
                        (Coach)Facade.currentClient,
                        sportSeance.getValue());
                super.goToMonEspace(updateSeance);
            }
        } catch (Exception e) {
            super.displayError(errorText, e.getMessage());
        }
    ...
    /**
     * @return true si les champs sont corrects, false sinon
     */
